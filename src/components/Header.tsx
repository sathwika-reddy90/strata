    ...
    <>
      <header className="fixed inset-x-0 top-0 z-50 px-4 pt-4 sm:px-8 sm:pt-6 lg:px-16 lg:pt-8 xl:px-24">
        <motion.div
          ...
          className={`relative mx-auto flex w-full max-w-[1320px] items-center justify-between gap-6 rounded-full border border-stone-200/70 bg-warm-white/80 pl-6 backdrop-blur-xl transition-all duration-500 sm:pl-8 ${
            scrolled ? "py-2.5 pr-4 bg-warm-white/92 shadow-[0_8px_30px_-12px_rgba(28,27,25,0.3)]" : "py-3.5 pr-5 shadow-[0_8px_30px_-14px_rgba(28,27,25,0.18)]"
          }`}
        >
          <Logo />

          <nav className="hidden lg:flex flex-1 items-center justify-center gap-4 xl:gap-6 2xl:gap-8">
            {primaryNav.map((item) => {
              ...
              return (
                ...
                >
                  <Link
                    to={item.href}
                    className={`relative whitespace-nowrap text-[12px] font-semibold uppercase tracking-[0.1em] transition-colors duration-300 hover:text-clay ${
          ...
          <div className="flex items-center gap-3">
            <a
              ...
              className="hidden md:inline-flex lg:hidden xl:inline-flex items-center gap-2 whitespace-nowrap rounded-full bg-clay px-5 py-2.5 text-[11px] font-semibold uppercase tracking-[0.14em] text-warm-white transition-all duration-300 hover:-translate-y-0.5 hover:bg-clay-deep"
            >
              Enquire Now
            </a>
            <button
